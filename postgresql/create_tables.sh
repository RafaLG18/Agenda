...
# Create DB almoxarifado
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
	CREATE USER alunoppoo WITH ENCRYPTED PASSWORD 't3sTing!';
	CREATE DATABASE almoxarifado OWNER alunoppoo;
EOSQL

export POSTGRES_USER="alunoppoo"
export PG_PASSWORD="t3sting!"
export POSTGRES_DB="almoxarifado"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
